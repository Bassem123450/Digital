...
$name = $position = $salary = $phone_number = $department_name = '';
$employee_id = null;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_dept_id->num_rows > 0) {
        ...
        if (isset($_POST['employee_id'])) {
            ...
            $sql_insert = "INSERT INTO Employees (name, position, salary, Phone_Number, department_id) VALUES ('$name', '$position', '$salary', '$phone_number', '$department_id')";

            if ($conn->query($sql_insert) === TRUE) {
                // Redirect to employees_dash.php after successful insertion
                header("Location: employees_dash.php");
                exit();
            } else {
...
// Fetch employee details if editing an existing employee
if(isset($_GET['id'])) {
    $employee_id = $_GET['id'];
...
<div class="container">
    ...
    <div class="row justify-content-center mt-5">
        <div class="col-md-6">
            <div class="card">
                <div class="card-body">
                    <h2 class="card-title text-center mb-4"><?php echo isset($_GET['id']) ? 'Edit Employee' : 'Add Employee'; ?></h2>
                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                        <!-- Employee details fields -->
                        <?php if(isset($_GET['id'])): ?>
                            <input type="hidden" name="employee_id" value="<?php echo $employee_id; ?>"> <!-- Add hidden input for employee_id if editing -->
                        <?php endif; ?>
                        ...
                        <div class="text-center">
                            <button type="submit" class="btn btn-primary btn-block"><?php echo isset($_GET['id']) ? 'Update' : 'Add'; ?></button>
                        </div>
                    </form>
                </div>
            </div>
